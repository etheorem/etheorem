...
| Function | Location | Property | Status | Tracking |
| --- | --- | --- | --- | --- |
| `shouldExtendPayload` | `Heze/ForkChoice.lean:320-342` | Landed: under successful preliminary lookup and slot checks, a verified payload with a recorded `false` inclusion-list satisfaction verdict is rejected by the FOCIL gate, with the pure runner state unchanged. Open: establish applicable insert/lookup and insert/contains laws. The generic `FcMap` interface provides neither, while the existing `treeMap` lookup theorem requires a `TransCmp Root` instance that `Root` does not have; use those laws with `onExecutionPayloadEnvelope_run_eq_of_successful_checks` to show that the handler-produced store exposes the inserted payload and satisfaction verdict, including a satisfaction lookup returning `some false` and `isPayloadVerified = true`; then compose those postconditions with `shouldExtendPayload_run_eq_false_of_recorded_unsatisfied`. The later Gloas-derived accept and reject paths and the missing-record `assert` branch also remain open | in progress | #63, `Proofs/Heze/ShouldExtendPayload.lean` |
...
| `onExecutionPayloadEnvelope` | `Heze/ForkChoice.lean:426-448` | `onExecutionPayloadEnvelope_run` is the complete compositional `.run` equation at `ForkChoiceStoreRun (Store map)`: missing `blockStates` is `.assert "envelope.beacon_block_root in store.block_states"`; failed data availability is `.assert "(isDataAvailable envelope.beaconBlockRoot)"`; verification and recorder errors propagate unchanged; success is the handler's final `set` of `blockStates` and `payloads` on the recorder's explicitly returned store, discarding the recorder-produced runner state. `onExecutionPayloadEnvelope_run_eq_of_successful_checks` restates the successful branch as the original store updated by three same-root `FcMap.insert` expressions. Lookup-after-insert, contains-after-insert, a later satisfaction lookup returning `some false`, `isPayloadVerified` on the handler-produced store, and composition with `shouldExtendPayload_run_eq_false_of_recorded_unsatisfied` remain on the `shouldExtendPayload` row | proved | follow-up to #84, `Proofs/Heze/OnExecutionPayloadEnvelope.lean` (successful-path origin #84) |
