...
| `processDeposit` | `Fulu/Operations.lean:230` | `validators.size = balances.size` holds at the append site, and then holds across the transition. The two overflow bounds are the separate rows under Bounds and termination properties | proposed |  |
