...
- The open `processDeposit` rows rest on the Merkle branch check. Its proof module is
...
| --- | --- | --- | --- | --- |
| `convertBuilderIndexToValidatorIndex` | `Gloas/Operations.lean:457` | Round-trips with `toBuilderIndex` on any `bi` that does not already carry the `BUILDER_INDEX_FLAG` bit, since `toBuilderIndex` always clears that bit, the round trip holds only under that precondition, not as a free identity | proved | #16, `Proofs/Gloas/BuilderIndex.lean` |
...
| `isValidIndexedPayloadAttestation` | `Gloas/Operations.lean:423-434` | Returns true exactly when the attesting indices are non-empty, adjacent-nondecreasing (duplicates permitted), all within the validator registry, and the configured `[CryptoBackend]` accepts the exact aggregate-verification call | proved | #38, `Proofs/Gloas/IsValidIndexedPayloadAttestation.lean` |
| `processPayloadAttestation` | `Gloas/Operations.lean:439-452` | Running `processPayloadAttestation` does not modify `State`: it only reads the input state and evaluates three assertions. A concrete run-level theorem should show that both success and rejection preserve the state component | proposed |  |
| `processExecutionPayloadBid` | `Gloas/Operations.lean:493-527` | The self-build and builder-bid paths it chooses between are mutually exclusive and jointly exhaustive | proposed |  |
| `applyParentExecutionPayload` | `Gloas/Operations.lean:532-556` | Exactly one of settle-current, settle-previous, or evict fires, so a payment is never settled twice | proposed |  |
...
| `processDeposit` | `Fulu/Operations.lean:230` | The index increment faults with `.arithmetic` only at `2^64 - 1`. With a valid branch and a smaller index, the run is `applyDeposit` on the state with the index incremented. An invalid branch rejects first. `processOperations` runs a deposit only while the index is below `eth1_data.deposit_count`, so the increment never faults there. The theorems say nothing about `applyDeposit` | proved | `Proofs/Fulu/DepositIndex.lean` |
| `processDeposit` | `Fulu/Operations.lean:230` | The running total balance never exceeds `2^64`. Dafny stated this bound and assumed it through `{:axiom}` lemmas, so Dafny's statement is reusable as a template. Its proofs are not. In Fulu `applyDeposit` registers a new validator at balance `0` and queues the amount in `pendingDeposits`, so the bound concerns the pending-deposit path. A sharper bound is open as well. The branch check needs `eth1DepositIndex < 2^32`, which follows from `eth1DepositIndex <= eth1Data.depositCount` together with a bound on `depositCount`. The consensus spec does not bound `depositCount`, since that count arrives from the execution layer. Any statement here is therefore conditional on the deposit contract's depth-32 capacity | proposed |  |
...
| `processDeposit` | `Fulu/Operations.lean:230` | Every successful run keeps `validators.size = balances.size` when it held before. `addValidatorToRegistry` and `applyDeposit` keep it too. The equality is what makes the unconditional pushes agree with the spec's `set_or_append_list`. At the registry limit both lists clamp together. The rest of the state transition is open. The two overflow bounds are the separate rows under Bounds and termination properties | proved | `Proofs/Fulu/RegistrySize.lean` |
| `addValidatorToRegistry` | `Fulu/Deposits.lean:31` | The run never rejects. It pushes onto `validators`, `balances`, both participation lists, and `inactivityScores`. `SSZList.push` clamps at the cap, where the pyspec raises. That gap is not modeled | proved | `Proofs/Fulu/RegistrySize.lean` |
