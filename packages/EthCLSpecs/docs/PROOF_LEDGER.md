...
| Function | Location | Property | Status | Tracking |
| --- | --- | --- | --- | --- |
| `shouldExtendPayload` | `Heze/ForkChoice.lean:320-342` | Landed: under successful preliminary lookup and slot checks, a verified payload with a recorded `false` inclusion-list satisfaction verdict is rejected by the FOCIL gate, with the pure runner state unchanged. Open: the later Gloas-derived accept and reject paths; the missing-record `assert` branch; lookup-after-insert and contains-after-insert on a store produced by `onExecutionPayloadEnvelope`, including a later satisfaction lookup returning `some false` and `isPayloadVerified` on that store; composition of that store with `shouldExtendPayload_run_eq_false_of_recorded_unsatisfied`; the generic `FcMap` interface provides no insert/lookup or insert/contains law; the `treeMap` lookup theorem requires a `TransCmp Root` instance that `Root` does not have | in progress | #63, `Proofs/Heze/ShouldExtendPayload.lean` |
