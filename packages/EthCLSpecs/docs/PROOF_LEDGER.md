...
| `getBeaconCommittee` | `Fulu/Committees.lean:83` | At mainnet, an active-validator count in `[32, 2^22]` implies every committee size is in `(0, MAX_VALIDATORS_PER_COMMITTEE]`. Dafny proves the same bound in `ActiveValidatorBounds`. At minimal the bound holds for an active count in `[8, 65536]`, because `MAX_COMMITTEES_PER_SLOT` is `4` there. At a symbolic preset the size is at least `1` when the committee count fits in the active set, and at most the ceiling of `n / count` | proved | `Proofs/Fulu/CommitteeSize.lean` |
