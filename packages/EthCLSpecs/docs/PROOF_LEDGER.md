...
| `applyWithdrawals` | `Gloas/Withdrawals.lean:184-194` | A builder-flagged withdrawal decreases the builder's balance by at most its own balance, so the balance never goes negative. Proved over any withdrawals array: every successful run keeps the builder registry's size, and no builder balance ends above its start. The validator side is `decreaseBalance`'s, and this row does not characterize it | proved | `Proofs/Gloas/ApplyWithdrawals.lean` |
