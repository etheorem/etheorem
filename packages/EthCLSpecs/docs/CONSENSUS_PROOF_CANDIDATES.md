...
| `canBuilderCoverBid`                    | `Gloas/Operations.lean:419-422`     | **In review**, see `EthCLSpecs/Proofs/CanBuilderCoverBid.lean`. Its `Bool` result is characterized exactly using the implementation's computed `minBalance`; the caller-level obligation invariant remains a follow-up |
...
| `processExecutionPayloadBid`       | `Gloas/Operations.lean:451-484`      | The self-build and builder-bid paths are mutually exclusive and jointly exhaustive. On the successful builder-bid path, a caller-level follow-up to canBuilderCoverBid should prove that queuing the accepted bid preserves MIN_DEPOSIT_AMOUNT + pending obligations ≤ builder balance, under explicit pending-total no-overflow and ring-cell freshness hypotheses. |
