...
| `convertBuilderIndexToValidatorIndex` | `Gloas/Operations.lean:415`          | Round-trips with `toBuilderIndex` on any `bi` that does not already carry the `BUILDER_INDEX_FLAG` bit, `toBuilderIndex` always clears it, so the round trip needs that precondition, not a free identity            |
