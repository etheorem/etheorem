...
| `canBuilderCoverBid`             | `Gloas/Operations.lean:419-422`     | Pure `Bool` predicate with no state change. The direct theorem exactly characterizes its result using the implementation's computed `minBalance`: acceptance requires `minBalance ≤ builderBalance` and `bidAmount ≤ builderBalance - minBalance`. Any caller-level theorem that adding the bid keeps total obligations within balance belongs to `processExecutionPayloadBid` |
