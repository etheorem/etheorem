...
Both close by `rfl`. They exist to be `rw`/`simp` targets with a readable right-hand
...
The per-fork runner names stay in the per-fork directories. `GloasRun` is
`Proofs/Gloas/Run.lean`. `ForkChoiceStoreRun` is `Proofs/StoreRun.lean`.
-/
