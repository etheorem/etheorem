...
EMPTY node or the FULL node of its parent. This module proves two facts.

1. On the EMPTY edge, with the empty parent requests, `processParentExecutionPayload`
   does not change the state. The fact covers this one step of the block transition.
   With other parent requests, the model rejects with an `assert`. No theorem here
   proves the reject.
