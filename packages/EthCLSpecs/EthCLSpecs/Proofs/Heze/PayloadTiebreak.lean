...
from the pending node of the block to its EMPTY node. The conditions are all needed.
At a block that is not from the previous slot, the attestation weights decide, and the
tiebreaker is the payload status itself. On a weight tie there, FULL (`1`) wins against
EMPTY (`0`). The module does not prove a fact about the full walk.
...
theorems about FULL also use the last two hypotheses: the payload is verified, and the
recorded answer is `false`. The default `[ExecutionEngine]` instance answers `true`
for every payload. So only a non-default engine records `false`.
-/
