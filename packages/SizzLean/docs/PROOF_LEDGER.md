...
| `branch × merkleizer` | | `naiveLeafAt_gindexBits` at `Proofs/Merkle/Opening.lean:229` | `isValidMerkleBranch` accepts the opening of `Spec.merkleize H chunks depth` at `index`, given `index < 2 ^ depth`, `chunks.length ≤ 2 ^ depth`, and 32-byte chunks. The `_idx` form reads the leaf as `chunks[index]?`, padding included. Needs `CombineWidth32` (`Hasher/Class.lean`), since the check takes 32-byte siblings | proved | `Proofs/Merkle/Opening.lean`, `naiveLeafAt_gindexBits`, `naiveRootAt_size`, `naiveLeafAt_size`, `naiveOpeningAt_length`, `naiveOpeningAt_size`, `merkleize_size`; `EthCLLib/Proofs/MerkleBranch.lean`, `isValidMerkleBranch_of_merkleize`, `isValidMerkleBranch_of_merkleize_idx`. |
