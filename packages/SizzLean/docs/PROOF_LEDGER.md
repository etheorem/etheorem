...
| `roundtrip × value-guard` | all | `SSZ.roundtrip` at `Repr/Class.lean:192-203` | `SSZ.roundtrip` stays gated by `BasicSupported r.shape` and takes `EncodedFits r.shape (r.toRepr x)`. The gate keeps the decoder's schema-validity conditions, which the two zero-width side conditions carry | proved | `Repr/Class.lean` |
