...
| `roundtrip × value-guard` | all | `SSZ.roundtrip` at `Repr/Class.lean:194-203` | `SSZ.roundtrip` stays gated by `BasicSupported r.shape` and takes `EncodedFits r.shape (r.toRepr x)`. The gate keeps the decoder's schema-validity conditions, which the two zero-width side conditions carry | proved | `Repr/Class.lean` |
| `serialize_injective × user-surface` | all | `SSZ.serialize_injective` at `Repr/Class.lean:213-219` | Non-malleability on a user type: `SSZ.serialize x = SSZ.serialize y` gives `x = y`, under the gates `SSZ.roundtrip` takes. The spec-level theorem gives `toRepr x = toRepr y`, and the `to_from` law maps it back. The three standard axioms alone | proved | `Repr/Class.lean` |
...
| `roundtrip × fork-schemas` | all | | `SSZ.roundtrip` and `SSZ.serialize_injective` on the real fork containers: `BeaconState`, `BeaconBlockBody`, `BeaconBlock`, and `SignedBeaconBlock` for Fulu, Gloas, and Heze, and Heze's `SignedInclusionList`. Kernel `decide` proves the gate through `gates × decidable`, so the size bound is the only hypothesis left. The block and state theorems range over the two shipped presets. The `Preset` class carries no positivity proof for `epochsPerSlashingsVector` (or for Gloas's `ptcSize`), and a zero value breaks the gate. `SignedInclusionList` has no preset-sized vector, so its theorems hold at every preset. The three standard axioms alone | proved | `EthCLSpecs/Proofs/Fulu/Codec.lean`, `EthCLSpecs/Proofs/Gloas/Codec.lean`, `EthCLSpecs/Proofs/Heze/Codec.lean` |
...
| `gates × decidable` | all | `SSZType.checkBasicSupported` at `Spec/BasicSupportedDecide.lean:49-57`, `SSZType.decBasicSupported` at `Spec/BasicSupportedDecide.lean:177-180` | `BasicSupported` is decidable. `checkBasicSupported` mirrors the predicate one arm at a time, with one check per fixed and variable constructor pair. Soundness and completeness are two structural mutual blocks, so the kernel reduces the instance and `by decide` proves the gate on a closed shape with no compiler axiom. The caller no longer builds the constructor tree by hand | proved | `Spec/BasicSupportedDecide.lean` |
