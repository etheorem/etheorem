...
| `branch × nested-field` | | `foldOpening_append` at `Proofs/Merkle/Opening.lean:221` | The same for field `k₂` of a container held in field `k₁`, the shape of `FINALIZED_ROOT_GINDEX` and `EXECUTION_BLOCK_HASH_GINDEX`. The branch is the outer opening followed by the inner one, at depth `d₁ + d₂` and index `k₁ * 2 ^ d₂ + k₂`; `foldOpening_append` splits the fold there. The `_gindex` form reads `g` off `generalizedIndex_field_field`. `EthCLSpecs/Tests/GindexPins.lean` pins the spec's gindex constants on the Fulu and Gloas schemas | proved | `Proofs/Merkle/Opening.lean`, `foldOpening_append`; `Spec/GeneralizedIndex.lean`, `generalizedIndex_field_field`; `EthCLLib/Proofs/ContainerBranch.lean`, `isValidMerkleBranch_of_container₂`, `isValidMerkleBranch_of_container₂_gindex`. |
