<!-- Generated by `just sizzlean-comp-benchmark`, then committed by hand with
     the two notes below. Regenerate rather than edit: the driver rewrites
     everything from the "# SizzLean comparative benchmark" heading down. -->

# The recorded run

This file is one run of the comparative benchmark, kept as the reference
number for SizzLean against `ethereum/ssz-specs` and `lambdaclass/libssz`.
Reproduce it with `just sizzlean-comp-benchmark`, which builds every side,
fetches both comparables at their pinned revisions, and writes a fresh report
to `packages/SizzLean/bench/`. The method is stated in
[`scripts/compbench/README.md`](../../../scripts/compbench/README.md).

Two things about *this* run a reader has to know, because neither is visible
in the numbers.

**The tree carried an uncommitted change.** The measured checkout batched the
uncached Merkle walk: `Spec/HashTreeRoot.lean`'s `combineLayerAt` handed each
level to a new `Hasher.batchCombine`, which `Hasher Sha256` overrides with the
ISA-L multi-buffer primitive. Against the committed walk that is worth 9 to
11% off the SizzLean Pure first root, measured by interleaved A/B, and nothing
on SizzLean Fast, which roots through the cached path instead. Batching the
cached path is [issue #3](https://github.com/etheorem/etheorem/issues/3).

**The machine was not idle.** Two unrelated processes held about 2.5 of the 12
cores throughout. Every SizzLean row is inflated by roughly 20%: deserialize
reads 164 ms here against 136 ms on the same code on a quiet machine. All four
implementations ran back to back under the same load, so the comparison
between rows holds. The absolute figures do not.

Re-take this file on an idle machine, from a committed tree, when either
matters.
